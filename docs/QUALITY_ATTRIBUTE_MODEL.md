...
Status: non-canonical planning reference.

This document is a small foundation for personal-project PR review quality. It is not part of the active versioned protocol load order, does not define review-package schema fields, and does not override `BOOTSTRAP.md`, `protocol-manifest.yaml`, the active `PR_REVIEW_CONTRACT.md`, active policies, active schemas, or deterministic validators.

If this document conflicts with the active protocol, the active protocol wins. A future patch may promote selected rules into a new versioned protocol only with schema, fixture, validator, and release-lock updates.
...
Reason: A short non-canonical reference prevents duplicate active policy while giving later protocol work a stable starting point.
...
Decision: add_new_minimal_doc
Reason: Record as a pilot candidate without claiming active validator enforcement.
Files affected: `docs/QUALITY_ATTRIBUTE_MODEL.md`
...
Decision: add_new_minimal_doc
Reason: Add seed rules as planning/partial-enforcement candidates only.
Files affected: `docs/QUALITY_ATTRIBUTE_MODEL.md`
...
These attributes are review priorities, not new schema fields.
...
| Research-backed judgment | Current or unstable external behavior must be verified before making a best-practice claim. | capabilities, evidence records, findings, unverified areas | Planning-only until promoted into the active protocol. |
...
These seed rules are not a new active specialty framework. They are starting candidates for a later versioned protocol or validator-backed pilot.
...
Current status: Partially enforceable by schema fields and decision gates; not fully validator-proven.
...
Current status: Partially enforceable through coverage, findings, required checks, and red/yellow gates.
...
Current status: Validator-backed for existing repository/protocol artifacts.
...
Current status: Partially enforceable through checks, evidence records, and decision gates.
...
Current status: Planning-only; not yet active-protocol or validator-enforced.
...
- No active protocol behavior is changed by this planning reference.
