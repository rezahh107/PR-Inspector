...
from .planning_authority import PlanningAuthorityError, resolve_current_planning_artifacts, resolve_planning_authority
...
def _authority_diagnostic(exc: PlanningAuthorityError) -> Diagnostic:
    return diagnostic(exc.code, exc.path, exc.message)
...
def _validate_evidence_chain(registry: dict[str, Any], package: dict[str, Any], scope: dict[str, Any], latest_impact_ref: str) -> list[Diagnostic]:
...
def _validate_registered_artifacts(root: Path, registry: dict[str, Any], schemas: dict[str, dict[str, Any]]) -> tuple[list[Diagnostic], dict[str, dict[str, Any]], dict[str, list[tuple[str, dict[str, Any]]]]]:
    root = Path(root).resolve()
    output: list[Diagnostic] = []
    scopes: dict[str, dict[str, Any]] = {}
    impacts: dict[str, list[tuple[str, dict[str, Any]]]] = {}
    try:
        resolve_planning_authority(root)
        current = resolve_current_planning_artifacts(root, registry)
    except PlanningAuthorityError as exc:
        return ([_authority_diagnostic(exc)], scopes, impacts)
    for package in registry['work_packages']:
        ...
        if scope is None:
            continue
        scope_errors = validate_scope(scope, schemas['scope'], registry, scope_ref, root=root)
        output += scope_errors
        ...
        for index, (impact_ref, impact) in enumerate(loaded):
            ...
            output += validate_impact(impact, schemas['impact'], scope, registry, impact_ref, package_status=package['status'], is_latest=index == len(loaded) - 1)
    ...
    if current_scope is None:
        output.append(diagnostic('PINS-PLANNING-BINDING-004', f'/{current.scope_ref}', 'current registered Scope did not validate'))
    if not current_impacts or tuple((item[0] for item in current_impacts)) != current.impact_refs:
        output.append(diagnostic('PINS-PLANNING-BINDING-005', f'/work_packages/{current.work_package_id}/impact_refs', 'current registered Impact chain did not validate exactly'))
    return (sorted(set(output)), scopes, impacts)

def _full_task_completion_eligible(task_id: str, registry: dict[str, Any], scopes: dict[str, dict[str, Any]], impacts: dict[str, list[tuple[str, dict[str, Any]]]]) -> bool:
...
def _validate_task_completion_and_dependencies(registry: dict[str, Any], scopes: dict[str, dict[str, Any]], impacts: dict[str, list[tuple[str, dict[str, Any]]]]) -> list[Diagnostic]:
    output: list[Diagnostic] = []
    for index, task in enumerate(registry['tasks']):
        ...
        requires_dependencies = task['implementation_authorized'] or task['status'] in {'authorized', 'in_progress', 'implemented', 'complete'}
        if requires_dependencies:
            incomplete = [dependency for dependency in task['depends_on'] if not _full_task_completion_eligible(dependency, registry, scopes, impacts)]
            if incomplete:
...
def validate_planning_repository(root: Path) -> list[Diagnostic]:
    ...
    try:
        ...
        return [_authority_diagnostic(exc)]
