...
def derive_action_mode(pkg: dict[str, Any]) -> str:
    return project_decision(pkg)["next_action"]["kind"]
...
def render_owner_result(projection_or_package: dict[str, Any]) -> str:
    projection = project_decision(projection_or_package) if "decision" in projection_or_package else projection_or_package
...
def render_next_action_prompt(pkg: dict[str, Any], projection: dict[str, Any] | None = None) -> str:
    projection = projection or project_decision(pkg)
...
def build_review_artifacts(pkg: dict[str, Any], review_package_bytes: bytes | None = None) -> dict[str, str]:
...
def write_review_artifacts(pkg: dict[str, Any], output_dir: Path, review_package_bytes: bytes | None = None) -> dict[str, str]:
    ...
    artifacts = build_review_artifacts(pkg, review_package_bytes=review_package_bytes)
