...
MODEL_ACTIONS = {"repair", "repair_and_verify", "verify", "rerun_review"}
...
def _contains_token(prompt: str, value: Any) -> bool:
    ...
    if token in prompt:
        return True
    # Rendered scalar fields use backticks rather than JSON quoting.
...
def _required_evidence_and_tests(package: Mapping[str, Any], action: str) -> tuple[list[str], list[str]]:
    evidence: list[str] = []
    tests: list[str] = []
    findings = package.get("findings")
    ...
    for finding in findings:
        ...
        supported = finding.get("evidence_label") in {"REPRODUCED", "CODE_SUPPORTED"}
        if action in REPAIR_ACTIONS and not (blocking or supported):
            continue
        refs = finding.get("evidence_refs")
        if isinstance(refs, Sequence) and not isinstance(refs, (str, bytes)):
            evidence.extend(str(ref) for ref in refs if isinstance(ref, str))
...
def validate_prompt_semantics(
    ...
    if not prompt_required:
        if prompt is not None:
            diagnostics.append(_diag("projection forbids a next-action prompt"))
        return diagnostics
    if prompt is None or not prompt.strip():
        return [_diag("projection requires a non-empty actionable prompt")]
    text = prompt.strip()
    if text == HISTORICAL_PLACEHOLDER:
        diagnostics.append(_diag("historical PR #22 placeholder is forbidden"))
    prompt_kind = action.get("prompt_kind")
    is_human_handoff = prompt_kind in {"human_review_handoff", "specialist_review_handoff"}
    minimum_lines = 14 if is_human_handoff else 20
    if len(text.splitlines()) < minimum_lines:
        diagnostics.append(_diag("prompt is non-empty but operationally incomplete"))
    required_sections = HUMAN_HANDOFF_SECTIONS if is_human_handoff else MODEL_REQUIRED_SECTIONS
    for section in required_sections:
        if section not in prompt:
            diagnostics.append(_diag(f"missing required section {section}"))
    for fragment in PROFILE_COMMAND_FRAGMENTS:
        if fragment in prompt:
            diagnostics.append(_diag("profile-selection commands must remain a separate verified artifact"))
            break

    identity = package.get("review_identity")
    if not isinstance(identity, Mapping):
        diagnostics.append(_diag("canonical package has no structured review identity"))
    ...
    for name, value in required_identity.items():
        if value in {None, ""}:
            diagnostics.append(_diag(f"structured identity field {name} is missing"))
    ...
    if not isinstance(reason_codes, list) or not reason_codes:
        diagnostics.append(_diag("prompt-required action must carry canonical reason codes"))
    elif not is_human_handoff and _json_token(reason_codes) not in prompt:
        diagnostics.append(_diag("prompt does not bind canonical reason codes"))
    elif is_human_handoff and not all(code in prompt for code in reason_codes):
        diagnostics.append(_diag("human handoff does not bind canonical reason codes"))

    if kind in REPAIR_ACTIONS:
        if max_modify is not True:
            diagnostics.append(_diag("repair action must carry bounded modification authority"))
        if "Modify only files materially necessary" not in prompt:
            diagnostics.append(_diag("repair prompt lacks bounded scope authority"))
    if kind in NON_MODIFYING_ACTIONS or kind in {"verify", "rerun_review"}:
        if max_modify is not False:
            diagnostics.append(_diag("verification or fresh-review action must not authorize modification"))
        if "No code, file, commit, workflow, schema, test, documentation, or behavior modification is authorized." not in prompt:
            diagnostics.append(_diag("non-modifying action lacks an explicit modification prohibition"))
    if kind == "rerun_review":
        if "run a fresh PR Inspector review" not in prompt:
            diagnostics.append(_diag("rerun_review must request a fresh review"))
        if "Repair every confirmed defect" in prompt or "bounded repair lead" in prompt:
            diagnostics.append(_diag("rerun_review must not authorize repair"))
    if kind == "verify" and ("Repair every confirmed defect" in prompt or "bounded repair lead" in prompt):
    ...
    if (
        ...
        diagnostics.append(_diag("governance-only gaps cannot create technical repair authority"))

    finding_ids = _finding_ids(package)
    evidence_refs, required_tests = _required_evidence_and_tests(package, str(kind))
    reason_details = projection.get("reason_details")
    if (
        kind in REPAIR_ACTIONS
        and not finding_ids
        and not package.get("required_actions")
        and (not isinstance(reason_details, list) or not reason_details)
    ):
        diagnostics.append(_diag("repair prompt has no affected finding, structured required action, or canonical reason subject"))
    for finding_id in finding_ids:
        if finding_id not in prompt:
            diagnostics.append(_diag(f"prompt omits affected finding {finding_id}"))
    for ref in evidence_refs:
        if ref not in prompt:
            diagnostics.append(_diag(f"prompt omits evidence reference {ref}"))
    for test in required_tests:
        if _json_token(test) not in prompt:
            diagnostics.append(_diag("prompt omits a finding-required test"))

    if is_human_handoff:
        if "No merge or approval is performed by PR Inspector." not in prompt:
            diagnostics.append(_diag("human handoff omits merge/approval prohibition"))
        if "A model response, prompt execution, or generated summary cannot satisfy or claim this approval." not in prompt:
            diagnostics.append(_diag("human handoff does not preserve the human-approval boundary"))
    else:
        ...
        for prohibition in mandatory_prohibitions:
            if prohibition not in prompt:
                diagnostics.append(_diag(f"prompt omits prohibition: {prohibition}"))
        if "independently reviewed again" not in prompt:
            diagnostics.append(_diag("prompt omits mandatory fresh PR Inspector re-review"))
    return sorted(set(diagnostics))
