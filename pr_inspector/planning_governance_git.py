...
def parse_git_name_status(
    raw: str,
) -> tuple[list[str], dict[str, str], list[Diagnostic]]:
        ...
        if not token or index + path_count > len(fields):
            diagnostics.append(
                diagnostic(
                    ...
                    "/git/diff",
                    f"malformed name-status record: {token!r}",
                )
            )
            break
...
def validate_git_diff(
    ...
    try:
        ...
        scope = load_json_strict(root / current.scope_ref)
        ...
        if not _FULL_SHA.fullmatch(authoritative_base_sha):
            output.append(
                diagnostic(
                    ...
                    "/git/base",
                    (
                        "authoritative PR base must be an exact "
                        "40-character SHA"
                    ),
                )
            )
        if not _FULL_SHA.fullmatch(head_sha):
            output.append(
                diagnostic(
                    "PINS-SCOPE-DISCLOSURE-MISMATCH",
                    "/git/head",
                    "runtime Head must be an exact 40-character SHA",
                )
            )
        if scope.get("base_sha") != authoritative_base_sha:
            output.append(
                diagnostic(
                    ...
                    "/scope/base_sha",
                    (
                        "declared base does not equal authoritative "
                        "pull-request base"
                    ),
                )
            )
        ...
        resolved_base = _git(
            root, "rev-parse", f"{authoritative_base_sha}^{{commit}}"
        )
        ...
        if resolved_head != head_sha or checkout != head_sha:
            output.append(
                diagnostic(
                    "PINS-SCOPE-DISCLOSURE-MISMATCH",
                    "/git/head",
                    "runtime Head identity mismatch",
                )
            )

        merge_base = _git(
            root, "merge-base", authoritative_base_sha, head_sha
        )
        ...
        if merge_base != authoritative_base_sha:
            output.append(
                diagnostic(
                    ...
                    "/git/merge-base",
                    (
                        "authoritative base is not the exact merge base; "
                        f"observed {merge_base}"
                    ),
                )
            )
        try:
        ...
        output.append(
            diagnostic("PINS-SCOPE-BASE-MISMATCH", "/git", str(exc))
        )
        return sorted(set(output)), report
    ...
    for path in sorted(set(actual) - set(declared)):
        output.append(
            diagnostic(
                "PINS-SCOPE-UNDECLARED-PATH",
                f"/{path}",
                "changed path is not declared",
            )
        )
    for path in sorted(set(declared) - set(actual)):
        output.append(
            diagnostic(
                "PINS-SCOPE-DECLARED-PATH-UNCHANGED",
                f"/{path}",
                "declared path is unchanged",
            )
        )
    for path in actual:
        for pattern in scope.get("excluded_paths", []):
            if (
                _pattern_error(pattern) is None
                and pattern_matches(pattern, path)
            ):
                output.append(
                    diagnostic(
    ...
    if actual != declared:
        output.append(
            diagnostic(
                "PINS-SCOPE-DISCLOSURE-MISMATCH",
                "/git/diff",
                "actual paths differ from declared paths",
            )
        )
    if not output:
        report["status"] = "valid"
