...
_PROFILE_REASON_FROM_CANONICAL = {
...
def _profile_reason_codes(
    ...
    mapped = list(
        dict.fromkeys(
            _PROFILE_REASON_FROM_CANONICAL[code]
            for code in technical_codes
            if code in _PROFILE_REASON_FROM_CANONICAL
        )
    )
    if mapped or technical_status == _core.STATUS_GREEN:
        return mapped
    return [
        "required_technical_check_failed"
        if technical_status == _core.STATUS_RED
        else "incomplete_technical_scope"
    ]
...
def project_decision(
    ...
    technical_decision = {
        "status": (
            "GREEN"
            if technical_status == _core.STATUS_GREEN
            else "RED"
            if technical_status == _core.STATUS_RED
            else "YELLOW"
        ),
        "reason_codes": _profile_reason_codes(technical_codes, technical_status),
    }
    identity = pkg.get("review_identity") if isinstance(pkg.get("review_identity"), dict) else {}
    if inspection_profile == "minimal":
        governance_decision = {"status": "NOT_REQUESTED", "reason_codes": []}
    elif inspection_profile == "strict":
        evidence_matches = (
            is_verified_governance_evidence(governance_evidence)
            and governance_evidence.repository == identity.get("target_repository")
            and governance_evidence.pull_request_number == identity.get("pr_number")
            and governance_evidence.exact_head_sha == identity.get("reviewed_head_sha")
        )
        ...
        elif governance_evidence.merge_authorized:
            governance_decision = {"status": "VERIFIED", "reason_codes": []}
        else:
            governance_decision = {
                "status": "GAP_FOUND",
    ...
    governance_follow_up = {
        "kind": (
            "none"
            if governance_decision["status"] in {"NOT_REQUESTED", "VERIFIED"}
            else "access_limitation"
            if governance_decision["status"] == "NOT_VERIFIABLE"
...
def validate_projection_invariants(projection: dict[str, Any]) -> None:
    ...
    if technical.get("status") != expected_profile_status:
        raise ProjectionError("active technical decision contradicts canonical technical status")
    if technical.get("reason_codes") != _profile_reason_codes(
        projection["technical_status_reason_codes"],
        projection["technical_status"],
    ):
        raise ProjectionError("active technical reasons contradict canonical technical reasons")
    ...
    if overall.get("merge_governance_verified") is not (governance.get("status") == "VERIFIED"):
        raise ProjectionError("merge_governance_verified contradicts governance decision")
    if projection.get("inspection_profile") == "minimal" and governance != {"status": "NOT_REQUESTED", "reason_codes": []}:
        raise ProjectionError("Minimal governance must be NOT_REQUESTED")
    if follow_up.get("may_modify_code") is not False or follow_up.get("prompt_required") is not False:
