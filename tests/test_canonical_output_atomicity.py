...
def source():
    return github_pull_request_head_source(
        REPOSITORY, PR_NUMBER, token=None, api_version=API_VERSION
    )


def run_review(tmp_path: Path, monkeypatch, payloads=None, output=None):
    ...
    return complete_review(package_path, output, head_source=source()), output, value
...
def test_quarantine_cleanup_failure_is_reported_not_successful_rollback(
    ...
    monkeypatch.setattr(_official_complete.shutil, "rmtree", fail_quarantine_cleanup)
    result = complete_review(package_path, output, head_source=source())

    assert isinstance(result, IncompleteReview)
    assert {"PRI-COMPLETE-ROLLBACK-010", "PRI-COMPLETE-ROLLBACK-011"}.issubset(
        codes(result)
    )
    install_payloads(monkeypatch)
    verify_completed_review(output, head_source=source())
...
def test_publication_failure_does_not_escape_supported_api(tmp_path, monkeypatch):
    from pr_inspector import _official_complete
