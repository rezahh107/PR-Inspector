...
def _record_reference(record_id: str) -> dict:
    return {
        "record_type": "review_receipt_core",
        "record_id": record_id,
...
def _publication_attempt(
    status: str,
    *,
    receipt_validation_status: str | None = None,
) -> dict:
    payload = copy.deepcopy(_valid("publication_attempt"))
    payload["publication_status"] = status
    payload["receipt_validation_status"] = (
        receipt_validation_status
        if receipt_validation_status is not None
        else ("current_valid" if status == "published_verified" else "not_checked")
    )
    ...
    if status == "not_attempted":
        payload.update(
            ...
            readback_evidence_ref=None,
        )
    elif status == "publishing":
        payload.update(
            published_object_id=901,
        ...
        payload.update(
            published_object_id=902,
            published_at="2026-07-18T12:02:00Z",
            readback_verified_at=None,
            published_body_sha256="d" * 64,
            failure_reason="platform write failed",
            readback_status="FAILED",
            readback_evidence_ref={
                "evidence_id": "readback-failed",
                "evidence_type": "structured_record",
                "exact_identity": "attempt-902",
                "source_digest": "sha256:" + "e" * 64,
                "retrieval_method": "github_api",
            },
        )
    else:
        raise AssertionError(f"unsupported publication attempt status: {status}")
...
def _tool_authorizes_consequential_action(payload: dict) -> bool:
    parameter_statuses = {
        ...
        for item in payload.get("parameter_bindings", [])
    }
    claim_statuses = {
        item.get("binding_status")
        for item in payload.get("final_claim_bindings", [])
    }
    invocation = payload.get("invocation_record", {})
    verified_parameters = parameter_statuses <= {
        "VERIFIED_SOURCE_BOUND",
        "VERIFIED_TRANSFORMATION_BOUND",
    }
    verified_claims = claim_statuses <= {
        "VERIFIED_RESULT_BOUND",
        "VERIFIED_READBACK_BOUND",
    }
    return (
        bool(parameter_statuses)
        and verified_parameters
        and invocation.get("claimed_execution") is True
        and invocation.get("execution_status") == "success"
        and payload.get("readback_status")
        in {"VERIFIED", "READBACK_NOT_REQUIRED"}
        and verified_claims
    )
...
def _tool_supports_successful_completion(payload: dict) -> bool:
    return (
        _tool_authorizes_consequential_action(payload)
        and payload.get("execution_result", {}).get("result_identity") is not None
        and all(
            item.get("binding_status")
            in {"VERIFIED_RESULT_BOUND", "VERIFIED_READBACK_BOUND"}
            for item in payload.get("final_claim_bindings", [])
        )
    )
...
def test_publication_attempt_status_carrier_is_exact_and_separate() -> None:
    common = _load(SCHEMA_DIR / "common.schema.json")
    schema = _load(SCHEMA_DIR / "publication_attempt.schema.json")

    assert common["$defs"]["receipt_publication_status"]["enum"] == [
        "not_required",
        "not_attempted",
        "publishing",
        "published_verified",
    ...
    assert schema["$defs"]["publication_attempt_status"]["enum"] == [
        "not_attempted",
        "publishing",
        "published_verified",
        "publication_failed",
    ...
    assert (
        schema["properties"]["publication_status"]["$ref"]
        == "#/$defs/publication_attempt_status"
    )
    assert schema["properties"]["receipt_validation_status"]["enum"] == [
        "not_checked",
        "current_valid",
        "stale",
        "invalid",
...
def test_each_publication_attempt_state_has_a_distinct_valid_shape(
    status: str,
) -> None:
    _assert_valid("publication_attempt", _publication_attempt(status))


def test_verified_publication_may_have_conflicting_receipt_validation() -> None:
    payload = _publication_attempt(
        "published_verified",
        receipt_validation_status="conflicting",
    )
...
@pytest.mark.parametrize(
    "invalid_status",
    ["not_required", "RECEIPT_CONFLICT", "conflicting"],
)
def test_non_attempt_statuses_are_rejected(
    invalid_status: str,
) -> None:
    payload = _publication_attempt("published_verified")
    payload["publication_status"] = invalid_status
    _assert_invalid("publication_attempt", payload)


def test_publication_and_validation_statuses_cannot_substitute() -> None:
    payload = _publication_attempt("published_verified")
    payload["receipt_validation_status"] = "publication_failed"
    _assert_invalid("publication_attempt", payload)


def test_not_attempted_rejects_publication_metadata() -> None:
    payload = _publication_attempt("not_attempted")
    payload["published_object_id"] = 900
    _assert_invalid("publication_attempt", payload)


def test_not_attempted_rejects_verified_readback() -> None:
    payload = _publication_attempt("not_attempted")
    payload["readback_status"] = "VERIFIED"
    payload["readback_verified_at"] = "2026-07-18T12:00:05Z"
    payload["readback_evidence_ref"] = copy.deepcopy(
        _valid("publication_attempt")["readback_evidence_ref"]
    )
    _assert_invalid("publication_attempt", payload)


def test_publishing_rejects_verified_readback() -> None:
    payload = _publication_attempt("publishing")
    payload["readback_status"] = "VERIFIED"
    payload["readback_evidence_ref"] = copy.deepcopy(
        _valid("publication_attempt")["readback_evidence_ref"]
    )
...
def test_publishing_rejects_readback_verified_timestamp() -> None:
    payload = _publication_attempt("publishing")
    payload["readback_verified_at"] = "2026-07-18T12:00:05Z"
    _assert_invalid("publication_attempt", payload)


def test_publication_failed_requires_failure_reason() -> None:
    payload = _publication_attempt("publication_failed")
    payload["failure_reason"] = None
    _assert_invalid("publication_attempt", payload)


def test_publication_failed_rejects_verified_publication_tuple() -> None:
    payload = _publication_attempt(
        "published_verified",
        receipt_validation_status="invalid",
    )
    payload["publication_status"] = "publication_failed"
    payload["failure_reason"] = "platform reported failure"
    _assert_invalid("publication_attempt", payload)
...
def test_published_verified_requires_complete_proof(
    field: str,
    replacement: object,
) -> None:
    payload = _publication_attempt("published_verified")
...
def test_published_verified_rejects_failure_reason() -> None:
    payload = _publication_attempt("published_verified")
    payload["failure_reason"] = "contradictory failure"
    _assert_invalid("publication_attempt", payload)


@pytest.mark.parametrize(
    "status",
    ["not_attempted", "publishing", "publication_failed"],
)
def test_current_valid_requires_verified_publication_basis(
    status: str,
) -> None:
    payload = _publication_attempt(
        status,
        receipt_validation_status="current_valid",
    )
    _assert_invalid("publication_attempt", payload)


def test_conflicting_validation_requires_exact_conflict_reference() -> None:
    payload = _publication_attempt(
        "published_verified",
        receipt_validation_status="conflicting",
    )
...
def _policy_transition_identity_errors(payload: dict) -> list[str]:
    ...
    if payload.get("evaluation_policy_identity") != payload.get(
        "current_policy_identity"
    ):
        errors.append(
            "evaluation_policy_identity must equal current_policy_identity"
        )
    return errors
...
def test_policy_transition_identity_is_semantically_bound_to_current_policy() -> None:
    ...
    assert (
        schema["properties"]["evaluation_policy_role"]["const"]
        == "current_policy"
    )
    ...
    assert _policy_transition_identity_errors(mutated) == [
        "evaluation_policy_identity must equal current_policy_identity"
    ]
...
@pytest.mark.parametrize(
    "status",
    ["INVALID_SOURCE_DETACHED", "INVALID_UNDECLARED_TRANSFORMATION"],
)
def test_tool_parameter_failure_states_are_representable_but_not_authorizing(
    status: str,
) -> None:
    ...
    payload["final_claim_bindings"][0][
        "binding_status"
    ] = "VERIFIED_RESULT_BOUND"
...
def test_tool_readback_failures_are_representable_without_success_claims(
    readback_status: str,
) -> None:
    payload = _valid("tool_execution_attestation")
    payload["state_changed"] = False
...
def test_terminal_l0_is_rejected_but_general_change_class_preserved() -> None:
    ...
    classification = _load(
        SCHEMA_DIR / "classification_record.schema.json"
    )
    assert common["$defs"]["change_class"]["enum"] == [
        "L0",
        "L1",
        "L2",
        "L3",
    ...
    assert classification["$defs"]["classification_terminal_class"]["enum"] == [
        "L1",
        "L2",
        "L3",
        "L4",
    ...
    payload = _valid("classification_record")
    payload["classification_resolution_status"] = (
