...
def record() -> dict:
    return json.loads((ROOT / "fixtures/governance/verified-enforced.json").read_text(encoding="utf-8"))
...
def test_active_lifecycle_documentation_is_not_candidate_wording():
    ...
    forbidden = ("active candidate on the unmerged PR branch", "default branch remains authoritative until this PR is merged")
    for path in paths:
...
def test_pr12_history_preserves_uncertainty():
    current = (ROOT / "CURRENT_VERSION").read_text(encoding="utf-8").strip()
    text = (ROOT / f"protocols/{current}/PR_REVIEW_CONTRACT.md").read_text(encoding="utf-8")
...
def test_bot_commented_review_does_not_satisfy_human_approval():
    value = record()
    value["reviews"] = [{"reviewer": "review-bot", "state": "COMMENTED", "commit_id": HEAD, "is_bot": True, "is_author": False}]
...
def test_missing_repository_settings_is_insufficient_evidence():
    ...
    assert not evidence.merge_authorized
    assert evidence.conclusion == "merge readiness appears satisfied, but repository-level enforcement is unverified"
...
def test_specialist_review_without_identity_fails():
    value = record()
    value["specialist_review"] = {"required": True, "reviewer_identity_observed": False, "qualification_verified": False, "reviewer": None, "enforcement_status": "human_governance_required"}
    assert not verified(value).specialist_satisfied


def test_specialist_identity_without_qualification_is_honest():
    value = record()
    value["specialist_review"] = {"required": True, "reviewer_identity_observed": True, "qualification_verified": False, "reviewer": "independent-reviewer", "enforcement_status": "human_governance_required"}
    evidence = verified(value)
...
def test_governance_evidence_identity_is_exact_head_bound():
    with pytest.raises(GovernanceEvidenceError, match="exact target head"):
        verify_governance_record(record(), expected_repository="example/project", expected_pr_number=42, expected_head_sha="2" * 40)


def test_governance_behavioral_matrix_and_mutations_are_complete():
    current = (ROOT / "CURRENT_VERSION").read_text(encoding="utf-8").strip()
    text = (ROOT / f"protocols/{current}/policies/GOVERNANCE_BEHAVIORAL_RULE_COVERAGE.md").read_text(encoding="utf-8")
    mutations = json.loads((ROOT / "fixtures/governance/mutation-cases.json").read_text(encoding="utf-8"))
    required = {
        "PRR-DOC-LIFECYCLE-001", "PRR-GOV-CLAIM-001", "PRR-GOV-REVIEW-001",
        "PRR-GOV-SPECIALIST-001", "PRR-GOV-AUTHORIZATION-001", "PRR-GOV-BYPASS-001",
