...
Review one pull request against an exact base/head identity, determine material risk from explicit evidence, consider untrusted external review suggestions when available, and produce one canonical JSON package plus two deterministic human-readable views.

The reviewer MUST NOT invent access, requirements, execution, evidence, checks, findings, external review results, or successful results.
...
A suggestion may be accepted only when PR Inspector independently verifies it against inspected evidence, links it to at least one finding, and records a specific safe repair. Accepted bot suggestions still require PR Inspector evidence and are never authoritative by themselves.

Rejected, duplicate, deferred, insufficient-evidence, and out-of-scope suggestions MUST NOT appear as implementer repair instructions. Inaccessible external review sources MUST be represented as `insufficient_evidence` instead of being silently ignored.
...
When findings require a downstream developer or implementer model to repair the same pull request, the package MAY include `repair_handoff`.
...
Repair handoff references MUST point to existing `findings[].finding_id` values, and each affected rule ID MUST be attached to the referenced finding. A package with `repair_handoff` present MUST NOT be technically Green, because same-PR repair guidance means merge-blocking or verification work remains.

Accepted external suggestions may also render into the Repair Handoff, but only from `external_review_intake.suggestions` with `triage_decision: accepted`, verified evidence, linked finding IDs, and suggestion-specific repair instructions.

The handoff does not authorize broader repository changes, automatic merge, CI-success claims, or release-lock edits without evidence.
...
## 14. Canonical rule IDs

`PRR-SHA-001`, `PRR-STALE-001`, `PRR-SENS-001`, `PRR-EVID-001`, `PRR-EXEC-001`, `PRR-SCOPE-001`, `PRR-STATUS-001`, `PRR-INJECT-001`, `PRR-SECRET-001`, `PRR-FIND-001`, `PRR-OWNER-001`, `PRR-HANDOFF-001`, `PRR-CONSIST-001`, `PRR-UX-001`, `PRR-UNCERT-001`, `PRR-NOHIDDEN-001`, `PRR-LOCK-001`, `PRR-INTENT-001`, `PRR-EXTREVIEW-001`.

## 15. Enforcement status

Version 1.7.0 is schema-validated, semantically validated, fixture-tested, deterministically rendered, and release-lock protected. This enforcement applies only when supplied artifacts are processed by the included validators; unvalidated conversational text is not automatically verified.
