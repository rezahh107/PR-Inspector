...
**Version:** 1.9.1  
**Status:** Versioned protocol candidate; activation is determined from live `main`, `CURRENT_VERSION`, and GitHub history  
...
`implementation_complete` records that the bounded implementation work is finished. `technically_accepted` requires a verified `GREEN_TECHNICALLY_READY` review on the exact current head. `approval_complete` requires authoritative current-head human approval evidence when an external requirement activates it. `governance_enforced` requires independently observed GitHub repository settings that actually block merge. `merge_authorized` requires all active stronger predicates. `merged` is only an observed GitHub lifecycle fact; it is never inferred from repository JSON.
...
Existing protocol IDs remain valid. Feature enforcement includes:

`PRR-OWNER-MERGE-001`, `PRR-OWNER-TWO-LINE-001`, `PRR-DECISION-SOURCE-001`, `PRR-REASON-REGISTRY-001`, `PRR-VERIFY-NOMODIFY-001`, `PRR-STALE-NOREPAIR-001`, `PRR-RECIPIENT-BOUNDARY-001`, `PRR-MANIFEST-BYTES-001`, `PRR-CI-IDENTITY-001`, `PRR-EXACT-HEAD-CLAIM-001`, `PRR-PENDING-REREVIEW-001`, `PRR-PROMPT-INJECTION-001`, `PRR-GOV-PERSONAL-PROFILE-001`, `PRR-GOV-CLAIM-SEPARATION-001`, and `PRR-GOV-PROFILE-TRIGGER-001`.
...
Governance evidence conforms to `schemas/governance-evidence.schema.json`, but the normalized record is derived only from fresh sealed official GitHub API response payloads. A caller-supplied record, `verified_enforced` string, boolean specialist claim, or canonical-looking URL list cannot create the opaque capability. Required repository-hosted checks are bound to both context and configured GitHub App ID. The operational sequence CLI fetches the payload bundle, verifies it, binds the resulting evidence ID to the exact event, and passes the capability into sequence validation.

Under `personal_ai_operated_strong_governance_minimum_security`, those repository-hosted controls are `optional_hardening` for `GREEN_MERGE_RECOMMENDED` when valid `sequence_ci_enforced` coverage exists. They remain mandatory for any claim of `repository_settings_enforced` or `merge_authorized`, and whenever a documented higher-security trigger activates them.

## 18. Personal minimum-security profile

`policies/PERSONAL_MINIMUM_SECURITY_PROFILE.md` is the canonical profile carrier.

A bounded `GREEN_MERGE_RECOMMENDED` may be supported by current exact-head independent AI review, exact-head required CI, valid Scope Gate and Progress Gate evidence, stale-on-change behavior, sequence validation, no open Critical blocker, and no unsupported evidence claim. It does not automatically require a dedicated GitHub App or repository-hosted enforcement under the default profile.
...
## 19. Historical provenance of PR #12
...
## 20. Repository-settings boundary

Repository code validates evidence and rejects overclaims. GitHub-hosted rules prevent merge when configured. Human or specialist reviewers supply judgment only when an external requirement activates those roles. PR Inspector records and validates evidence but never approves or merges. Repository settings are administrative actions and are not changed by this protocol without explicit authorization.
