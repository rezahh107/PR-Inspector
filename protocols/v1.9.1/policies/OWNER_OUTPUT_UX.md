...
The profile evaluator may additionally state `GREEN_MERGE_RECOMMENDED`; that label is a bounded technical recommendation and never upgrades repository-settings, authorization, or merge facts.
...
Under the personal minimum-security profile, a missing GitHub App or missing branch protection does not turn this message Yellow by itself. The wording deliberately does not claim `repository_settings_enforced`, `merge_authorized`, or `merged`.
...
The Green result does not claim required reviews, required checks, bypass-resistant repository settings, merge authorization, or merge completion.
