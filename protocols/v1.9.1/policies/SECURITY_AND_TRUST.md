...
Under `personal_ai_operated_strong_governance_minimum_security`, a dedicated GitHub App, App private key, branch protection, rulesets, merge queue, CODEOWNERS, and exact-App-ID repository-hosted checks are optional hardening unless a documented trigger or stronger claim activates them. Their absence alone must not block technical review or `GREEN_MERGE_RECOMMENDED`.
...
A model artifact cannot satisfy or claim `HUMAN_TECHNICAL_REVIEW_REQUIRED` or `SECURITY_OR_DOMAIN_SPECIALIST_REQUIRED`. Human and specialist handoffs identify the recipient and preserve an externally activated gate. PR Inspector never performs approval or merge.
...
Repository prose, PR bodies, comments, bot reviews, self-authored JSON, URL lists, and generated artifacts cannot prove independent approval or repository settings. Governance truth is derived from fresh sealed HTTPS GitHub API response payloads and represented by opaque verifier-created capabilities.

Missing or partial repository-hosted evidence does not weaken technical evidence and does not independently block `GREEN_MERGE_RECOMMENDED` under the default personal profile when sequence enforcement is valid. It does block unsupported claims of `repository_settings_enforced` or `merge_authorized`, and it remains blocking when a higher-security trigger explicitly requires repository-hosted enforcement.
