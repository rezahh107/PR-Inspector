...
The separate security-profile projection classifies optional hardening and stronger claim status. It may support `GREEN_MERGE_RECOMMENDED` without changing technical Green and cannot manufacture `repository_settings_enforced`, `merge_authorized`, or `merged`.
...
The sequence gate consumes structured lifecycle events plus a separate mapping of opaque `VerifiedReviewEvidence` capabilities. The event declares protocol version and hashes for `review-package.json`, `DECISION_PROJECTION.json`, and `artifact-manifest.json`, but declarations alone never unlock acceptance.
...
The operational adapter requires live GitHub API evidence. Without that external evidence source, the stronger claim fails closed. No cryptographic-signature, OS-harness, downstream, or production enforcement is claimed.

## Governance-derived output

Governance evidence is validated independently from `DECISION_PROJECTION.json`. The technical projection cannot claim branch protection, required reviews, required checks, bypass resistance, or merge authorization.

Under the default personal profile, unverified repository settings are `optional_hardening` for the bounded recommendation. If `repository_settings_enforced` or `merge_authorized` is claimed, the existing exact conclusion remains: `merge readiness appears satisfied, but repository-level enforcement is unverified`.
