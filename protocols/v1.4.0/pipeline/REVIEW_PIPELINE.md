...
9. Examine realistic malformed, missing, stale, duplicate, oversized, partial, timeout, retry, concurrency, compatibility, migration, rollback, and resource-limit scenarios when connected to changed behavior.
10. Create `review-package.json` before human-readable outputs.
11. Run schema and semantic validation without suppressing diagnostics.
12. Render both Markdown artifacts deterministically.
13. Re-read the current PR head SHA. A change makes the review `STALE`; rebuild the package and outputs.
14. Emit Owner Card first, Technical Handoff second, and provide the JSON package as the canonical artifact.
