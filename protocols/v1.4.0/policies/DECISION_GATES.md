...
Return `GREEN_TECHNICALLY_READY` only when no Red or Yellow reason exists, validity is current, required checks passed with evidence, coverage is complete, no high-risk area is unreviewed, and implementation matches intended behavior.
