...
## Cross-accessor owner-delivery invariant

The general Behavioral Rule Coverage matrix tracks `PRR-OWNER-PROMPT-ATOMIC-001`. It proves that a prompt-required compact owner accessor cannot return owner text, ignored warning filters cannot bypass the invariant, the canonical accessor returns exact verified prompt bytes, no-prompt delivery remains exact, and CLI delivery failure leaves stdout empty.
