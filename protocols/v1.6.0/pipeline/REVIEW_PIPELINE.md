...
12. Add `repair_handoff` only when a finding needs concise downstream repair guidance for the same pull request.
13. Run schema and semantic validation without suppressing diagnostics.
14. Render both Markdown artifacts deterministically.
15. Re-read the current PR head SHA. A change makes the review `STALE`; rebuild the package and outputs.
16. Emit Owner Card first, Technical Handoff second, and provide the JSON package as the canonical artifact.
